...
def feature_engineer(X: pd.DataFrame, y: pd.Series):
    ...
    base_fea = [c for c in df.columns if c not in encoded_fea]
    ...
    Y_dict = {f"temp_t+{h}": df["temp"].shift(-h) for h in [1, 2, 3, 4, 5]}
    Y_df = pd.DataFrame(Y_dict, index=df.index)

    combined = pd.concat([df, Y_df], axis=1).dropna(axis=0)
