...
class Preprocessor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        # Keep datetime columns separately
        datetime_values = df[self.datetime_cols].copy() if all(
            col in df.columns for col in self.datetime_cols
        ) else pd.DataFrame()
        ...
        # Label encode categorical data - WITH UNSEEN LABEL HANDLING
        for col in self.categorical_cols_to_keep_:
            if col in temp_df.columns:
                # Convert to string
                col_data = temp_df[col].astype(str)
                
                # Get the fitted classes from the encoder
                known_classes = set(self.label_encoders_[col].classes_)
                
                # Find values in current data
                current_values = set(col_data.unique())
                
                # Find unseen labels (values not in training data)
                unseen_labels = current_values - known_classes
                
                if unseen_labels:
                    # Handle unseen labels
                    print(f"   ⚠️  Column '{col}': Found {len(unseen_labels)} unseen labels: {unseen_labels}")
                    
                    # Strategy: Map to the first/most frequent known class
                    most_frequent_class = self.label_encoders_[col].classes_[0]
                    
                    # Replace all unseen labels
                    col_data = col_data.replace(list(unseen_labels), most_frequent_class)
                    
                    print(f"      → Mapping to '{most_frequent_class}'")
                
                # Now transform (all values should be known)
                temp_df[col] = self.label_encoders_[col].transform(col_data)
