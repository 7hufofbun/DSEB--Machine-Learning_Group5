...
if __name__ == "__main__":
    ...
    path = "https://raw.githubusercontent.com/7hufofbun/DSEB--Machine-Learning_Group5/main/data/weather_hcm_daily.csv"
    data = get_data(path)
    ...
    save_rf_models_to_onnx(pipeline, save_dir="models_onnx/")
            # ==============================
    #  💾 SAVE FEATURE SCHEMA (5 files)
    # ==============================
    import json, os
    os.makedirs("schemas", exist_ok=True)

    for h in [1, 2, 3, 4, 5]:
        model_info = pipeline["models"][f"y+{h}"]
        feature_names = model_info["feature_names"]
        with open(f"schemas/features_y+{h}.json", "w") as f:
            json.dump({"feature_names": feature_names}, f, indent=2)

    print("✅ Saved 5 feature schema files into /schemas/")
