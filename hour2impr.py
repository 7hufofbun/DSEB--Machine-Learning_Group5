...
# ============================================================================
# CONFIGURATION - SPECIALIZED FOR D+2
# ============================================================================
...
print("="*80)
print("SPECIALIZED D+2 TEMPERATURE PREDICTION")
print("="*80)
print("Focus: Predicting temperature 2 days ahead")
print("Strategy: Custom feature engineering for 48-hour forecasting")
print("="*80)
...
# ============================================================================
# 2. PREPARE DAILY TARGET WITH D+2 SPECIFIC FEATURES
# ============================================================================
print("\n" + "="*80)
print("PREPARING DAILY TARGET WITH D+2 SPECIFIC FEATURES")
print("="*80)
...
# D+2 specific: 2-day patterns
daily_target['temp_2d_ago'] = daily_target['daily_temp'].shift(2)
daily_target['temp_diff_2d'] = daily_target['daily_temp'].diff(2)
daily_target['temp_change_rate_2d'] = daily_target['temp_diff_2d'] / 2

# Multi-day patterns (1-7 days)
...
# ============================================================================
# 4. SPECIALIZED FEATURE ENGINEERING FOR D+2
# ============================================================================
def engineer_features_for_d2(df):
    """Specialized feature engineering for 48-hour (D+2) forecasting"""
    df_eng = df.copy()
    ...
    # D+2 SPECIFIC: TEMPERATURE FEATURES
    if 'temp' in df_eng.columns:
        # Longer lag features (more relevant for 48h forecast)
        for lag in [1, 2, 3, 6, 12, 24, 36, 48]:
            df_eng[f'temp_lag_{lag}h'] = df_eng['temp'].shift(lag)
        
        # Rolling windows optimized for D+2 (12h to 48h)
        for window in [12, 24, 36, 48, 72]:
        ...
        df_eng['temp_momentum_6h'] = df_eng['temp'] - df_eng['temp'].shift(6)
        df_eng['temp_momentum_12h'] = df_eng['temp'] - df_eng['temp'].shift(12)
        df_eng['temp_momentum_24h'] = df_eng['temp'] - df_eng['temp'].shift(24)
        
        # Temperature acceleration
        df_eng['temp_accel_12h'] = df_eng['temp_momentum_6h'] - df_eng['temp_momentum_6h'].shift(6)
        df_eng['temp_accel_24h'] = df_eng['temp_momentum_12h'] - df_eng['temp_momentum_12h'].shift(12)
        ...
        # Diurnal cycle strength
        df_eng['is_daytime'] = ((df_eng.index.hour >= 6) & (df_eng.index.hour <= 18)).astype(int)
    ...
    if 'pressure' in df_eng.columns:
        ...
        df_eng['pressure_change_48h'] = df_eng['pressure'] - df_eng['pressure'].shift(48)
...
print("\n" + "="*80)
print("SPECIALIZED FEATURE ENGINEERING FOR D+2")
print("="*80)

hourly_train_eng = engineer_features_for_d2(hourly_train)
hourly_val_eng = engineer_features_for_d2(hourly_val)
hourly_test_eng = engineer_features_for_d2(hourly_test)
...
def aggregate_hourly_to_daily(df_hourly):
    """Aggregate with emphasis on recent patterns"""
    df = df_hourly.copy()
    ...
    for col in df.columns:
        ...
        elif any(stat in col for stat in ['mean', 'std', 'min', 'max', 'range', 'cv', 'momentum', 'accel']):
            agg_functions[col] = 'mean'
        elif col in ['is_weekend', 'day_of_week', 'month', 'day_of_year', 'week_of_year', 'quarter', 'is_daytime']:
...
# ============================================================================
# 6. PREPARE D+2 TRAINING DATA
# ============================================================================
print("\n" + "="*80)
print("PREPARING D+2 PREDICTION DATA")
print("="*80)

# Shift features by 2 days to predict D+2
train_features = daily_train_features.copy()
val_features = daily_val_features.copy()
test_features = daily_test_features.copy()

train_features.index = pd.to_datetime(train_features.index) + pd.Timedelta(days=2)
val_features.index = pd.to_datetime(val_features.index) + pd.Timedelta(days=2)
test_features.index = pd.to_datetime(test_features.index) + pd.Timedelta(days=2)
...
exclude_cols = [TARGET_COL] + [col for col in train_data.columns if 'temp_diff' in col or ('temp_std' in col and 'd' in col) or ('temp_range' in col and 'd' in col) or ('temp_mean' in col and 'd' in col) or ('temp_trend' in col and 'd' in col) or col in ['temp_2d_ago', 'temp_change_rate_2d', 'day_of_week']]
...
# ============================================================================
# 7. TRAIN D+2 MODEL WITH OPTIMIZATION
# ============================================================================
print("\n" + "="*80)
print("TRAINING D+2 MODEL")
print("="*80)
...
print("\n" + "="*80)
print("MODEL TRAINING COMPLETE - D+2")
print("="*80)
