...
# ============================================================================
# CONFIGURATION - SPECIALIZED FOR D+3
# ============================================================================
...
print("="*80)
print("SPECIALIZED D+3 (3 DAYS AHEAD) TEMPERATURE PREDICTION")
print("="*80)
print("Focus: Predicting temperature 72 hours ahead")
print("Strategy: Weekly patterns, weather system cycles")
print("="*80)
...
# ============================================================================
# 2. PREPARE DAILY TARGET WITH D+3 SPECIFIC FEATURES
# ============================================================================
print("\n" + "="*80)
print("PREPARING DAILY TARGET WITH D+3 SPECIFIC FEATURES")
print("="*80)
...
# D+3 specific: 3-day patterns and weekly cycles
daily_target['temp_3d_ago'] = daily_target['daily_temp'].shift(3)
daily_target['temp_diff_3d'] = daily_target['daily_temp'].diff(3)
daily_target['temp_change_rate_3d'] = daily_target['temp_diff_3d'] / 3

# Weekly pattern
daily_target['temp_7d_ago'] = daily_target['daily_temp'].shift(7)
daily_target['temp_diff_7d'] = daily_target['daily_temp'].diff(7)

# Multi-day patterns (3-7 days for medium-term)
...
daily_target['temp_volatility_5d'] = daily_target['daily_temp'].rolling(
    window=5, min_periods=1).std() / (daily_target['daily_temp'].rolling(
    window=5, min_periods=1).mean() + 1e-6)
...
# ============================================================================
# 4. SPECIALIZED FEATURE ENGINEERING FOR D+3
# ============================================================================
def engineer_features_for_d3(df):
    """Specialized feature engineering for 72-hour (D+3) forecasting"""
    df_eng = df.copy()
    ...
    # Temporal features (weekly focus)
    ...
    df_eng['is_monday'] = (df_eng.index.dayofweek == 0).astype(int)
    df_eng['is_friday'] = (df_eng.index.dayofweek == 4).astype(int)
    ...
    # D+3 SPECIFIC: MEDIUM-TERM TEMPERATURE FEATURES
    if 'temp' in df_eng.columns:
        # Medium-term lags (24-72 hours + weekly)
        for lag in [12, 24, 36, 48, 60, 72]:
            df_eng[f'temp_lag_{lag}h'] = df_eng['temp'].shift(lag)
        
        # Weekly lag
        df_eng['temp_lag_168h'] = df_eng['temp'].shift(168)
        
        # Rolling windows optimized for D+3 (24h to 7 days)
        for window in [24, 36, 48, 72, 96, 120, 168]:
        ...
        df_eng['temp_trend_24h'] = df_eng['temp'] - df_eng['temp'].shift(24)
        ...
        df_eng['temp_momentum_3d'] = (df_eng['temp'] - df_eng['temp'].shift(72)) / 3
        df_eng['temp_momentum_5d'] = (df_eng['temp'] - df_eng['temp'].shift(120)) / 5
        
        # Temperature variability
        for window in [48, 72, 120]:
            mean = df_eng[f'temp_mean_{window}h']
            std = df_eng[f'temp_std_{window}h']
            df_eng[f'temp_cv_{window}h'] = std / (mean + 1e-6)
        
        # Weekly comparison
        df_eng['temp_vs_last_week'] = df_eng['temp'] - df_eng['temp_lag_168h']
    
    # Weather system features (medium-term)
    if 'pressure' in df_eng.columns:
        for window in [24, 48, 72, 120]:
            df_eng[f'pressure_mean_{window}h'] = df_eng['pressure'].rolling(
                window=window, min_periods=1).mean()
            df_eng[f'pressure_std_{window}h'] = df_eng['pressure'].rolling(
                window=window, min_periods=1).std()
        
        df_eng['pressure_trend_24h'] = df_eng['pressure'] - df_eng['pressure'].shift(24)
        df_eng['pressure_trend_48h'] = df_eng['pressure'] - df_eng['pressure'].shift(48)
        df_eng['pressure_trend_72h'] = df_eng['pressure'] - df_eng['pressure'].shift(72)
    ...
    if 'humidity' in df_eng.columns:
        for window in [24, 48, 72]:
            df_eng[f'humidity_mean_{window}h'] = df_eng['humidity'].rolling(
                window=window, min_periods=1).mean()
            df_eng[f'humidity_std_{window}h'] = df_eng['humidity'].rolling(
                window=window, min_periods=1).std()
    ...
    if 'windspeed' in df_eng.columns:
        for window in [24, 48, 72]:
            df_eng[f'windspeed_mean_{window}h'] = df_eng['windspeed'].rolling(
                window=window, min_periods=1).mean()
            df_eng[f'windspeed_max_{window}h'] = df_eng['windspeed'].rolling(
                window=window, min_periods=1).max()
    ...
    if 'humidity' in df_eng.columns and 'temp' in df_eng.columns:
        for window in [24, 48, 72]:
            ...
            df_eng[f'apparent_temp_{window}h'] = temp_mean + 0.5 * (humidity_mean / 100 - 0.1) * (temp_mean - 14)
...
print("\n" + "="*80)
print("SPECIALIZED FEATURE ENGINEERING FOR D+3")
print("="*80)

hourly_train_eng = engineer_features_for_d3(hourly_train)
hourly_val_eng = engineer_features_for_d3(hourly_val)
hourly_test_eng = engineer_features_for_d3(hourly_test)
...
def aggregate_hourly_to_daily(df_hourly):
    """Aggregate with emphasis on multi-day patterns"""
    df = df_hourly.copy()
    ...
    for col in df.columns:
        ...
        elif any(stat in col for stat in ['mean', 'std', 'min', 'max', 'range', 'cv', 'trend', 'momentum']):
            agg_functions[col] = 'mean'
        elif col in ['is_weekend', 'is_weekday', 'is_monday', 'is_friday', 
...
# ============================================================================
# 6. PREPARE D+3 TRAINING DATA
# ============================================================================
print("\n" + "="*80)
print("PREPARING D+3 PREDICTION DATA")
print("="*80)

# Shift features by 3 days
train_features = daily_train_features.copy()
val_features = daily_val_features.copy()
test_features = daily_test_features.copy()

train_features.index = pd.to_datetime(train_features.index) + pd.Timedelta(days=3)
val_features.index = pd.to_datetime(val_features.index) + pd.Timedelta(days=3)
test_features.index = pd.to_datetime(test_features.index) + pd.Timedelta(days=3)
...
exclude_cols = [TARGET_COL] + [col for col in train_data.columns if 'temp_diff' in col or ('temp_std' in col and 'd' in col) or ('temp_range' in col and 'd' in col) or ('temp_mean' in col and 'd' in col) or ('temp_trend' in col and 'd' in col) or col in ['temp_3d_ago', 'temp_7d_ago', 'temp_change_rate_3d', 'temp_volatility_5d', 'day_of_week', 'is_weekend']]
...
if TUNE_HYPERPARAMETERS and OPTUNA_AVAILABLE:
    ...
    print("Training with default parameters optimized for D+3...")
    model = XGBRegressor(
        objective='reg:squarederror',
        n_estimators=300,
        learning_rate=0.07,
...
print("\n" + "="*80)
print("MODEL TRAINING COMPLETE - D+3")
print("="*80)
